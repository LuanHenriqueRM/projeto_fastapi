...
async def listar_pedidos(session: Session = Depends(pegar_sessao), usuario: Usuario = Depends(verificar_token)):
    if not usuario.admin:
        raise HTTPException (status_code=401, detail="Acesso negado! Você não tem autorização para realizar essa operação.")
    else:
...
# Listar todos os pedidos do usuário (Será exibido os pedidos do usuário que estiver logado!)   
